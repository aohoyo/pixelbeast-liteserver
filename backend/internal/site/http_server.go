...
func (s *HTTPServer) logHTTPError(method, path, remoteAddr, errMsg string, statusCode int) {
	logger.LogHTTPError(method, path, remoteAddr, errMsg, statusCode)
	if s.SiteID != "" {
		logger.LogHTTPErrorBySite(s.SiteID, method, path, remoteAddr, errMsg, statusCode)
	}
}
...
func (s *HTTPServer) logRequest(r *http.Request, start time.Time, status int) {
	...
	if s.SiteID != "" {
		logger.LogHTTPAccessBySite(s.SiteID, r.Method, r.URL.Path, r.RemoteAddr, status, duration)
	}
}
