...
type FTPServer struct {
	...
	clients   sync.Map          // key: net.Conn, value: bool
	mu        sync.Mutex        // 保护 userConns
	running   bool
	rootDir   string
...
func (s *FTPServer) Start() error {
	...
	logger.LogPanelRuntime(logger.LogLevelInfo, "[FTP] 服务器启动在端口 %d", s.Config.Port)
	logger.LogPanelRuntime(logger.LogLevelInfo, "[FTP] 根目录: %s", s.rootDir)
...
func (s *FTPServer) acceptConnections() {
	for {
		...
		if err != nil {
			if s.running {
				logger.LogFTPError("", "", "接受连接", err.Error())
			}
			return
		}
...
func (s *FTPServer) handleClient(conn net.Conn) {
	...
	logger.LogFTPConnection(conn.RemoteAddr().String(), true)
	...
	for {
		...
		logger.LogFTPCommand(client.username, conn.RemoteAddr().String(), cmd, args)
		client.handleCommand(cmd, args)
	}

	logger.LogFTPConnection(conn.RemoteAddr().String(), false)
}
...
type FTPClient struct {
	...
	dataConnReady   chan struct{} // PASV 数据连接就绪信号
...
func (c *FTPClient) handlePASS(password string) {
	...
	if c.server.validator == nil {
		...
		logger.LogFTPLogin(c.username, c.conn.RemoteAddr().String(), false, "验证器未配置")
		c.username = ""
		return
	}
	...
	if !c.server.validator.ValidateFTPUser(c.username, password) {
		...
		logger.LogFTPLogin(c.username, c.conn.RemoteAddr().String(), false, "密码错误")
		c.username = ""
		return
	}
	...
	if userCfg == nil {
		...
		logger.LogFTPLogin(c.username, c.conn.RemoteAddr().String(), false, "用户不存在或已禁用")
		c.username = ""
		return
	}
	...
	if userCfg.ExpiryDate != "" {
		if expiryTime, err := time.Parse("2006-01-02", userCfg.ExpiryDate); err == nil {
			if time.Now().After(expiryTime) {
				...
				logger.LogFTPLogin(c.username, c.conn.RemoteAddr().String(), false, "账号已过期")
				c.username = ""
				return
			}
		}
	...
	if userCfg.MaxConnections > 0 {
		...
		if current >= userCfg.MaxConnections {
			...
			logger.LogFTPLogin(c.username, c.conn.RemoteAddr().String(), false, "超过最大连接数")
			c.username = ""
			return
		}
	...
	logger.LogFTPLogin(c.username, c.conn.RemoteAddr().String(), true, "登录成功")
}
...
func (c *FTPClient) handlePASV() {
	...
	c.dataConnReady = make(chan struct{}, 1)
...
func (c *FTPClient) handleRETR(filename string) {
	...
	info, err := os.Stat(fullPath)
	if err != nil || info.IsDir() {
		logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "ä¸è½½ "+filename, "æä»¶ä¸å­å¨")
	...
	if userCfg := c.getUserConfig(); userCfg != nil && userCfg.MaxFileSize > 0 {
		if info.Size() > userCfg.MaxFileSize*1024*1024 {
			logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "ä¸è½½ "+filename, "è¶è¿åæä»¶å¤§å°éå¶")
	...
	dataConn, err := c.getDataConnection()
	if err != nil {
		logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "ä¸è½½ "+filename, err.Error())
		c.sendMessage("425 Cannot open data connection")
		return
	}
	defer dataConn.Close()
	...
	if err != nil {
		logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "下载 "+filename, err.Error())
	...
	logger.LogFTPTransfer(c.username, c.conn.RemoteAddr().String(), filename, "下载", transferred, duration, true)
	c.sendMessage("226 Transfer complete")
}
...
func (c *FTPClient) handleSTOR(filename string) {
	...
	if userCfg != nil && userCfg.MaxFiles > 0 {
		if c.countFiles(c.rootDir) >= userCfg.MaxFiles {
			logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "ä¸ä¼  "+filename, "è¶è¿æä»¶æ°ééå¶")
	...
	dataConn, err := c.getDataConnection()
	if err != nil {
		logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "ä¸ä¼  "+filename, err.Error())
		c.sendMessage("425 Cannot open data connection")
		return
	}
	defer dataConn.Close()
	...
	if err != nil {
		logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "上传 "+filename, err.Error())
	...
	if sizeExceeded {
		os.Remove(fullPath)
		logger.LogFTPError(c.username, c.conn.RemoteAddr().String(), "上传 "+filename, "超过单文件大小限制")
		c.sendMessage("553 File too large")
		return
	}

	duration := time.Since(start)
	logger.LogFTPTransfer(c.username, c.conn.RemoteAddr().String(), filename, "上传", totalSize, duration, true)
	c.sendMessage("226 Transfer complete")
}
...
func (c *FTPClient) getDataConnection() (net.Conn, error) {
	...
	c.transferMu.Unlock()
	// 主动连接
	return net.DialTimeout("tcp", c.dataPort, 10*time.Second)
}
