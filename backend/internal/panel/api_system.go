...
import (
	...
	"pixelbeast/backend/internal/monitor"
)
...
func (h *Handler) getSystemStatus(w http.ResponseWriter, r *http.Request) {
	...
	Success(w, map[string]interface{}{
		...
		"sites_running": h.SiteManager != nil && h.SiteManager.IsSitesRunning(),
...
func (h *Handler) freeMemory(w http.ResponseWriter, r *http.Request) {
	...
	// 3. 平台特定的系统内存释放
	osErr := monitor.FreeSystemMemory()
	...
	logger.LogPanelOperation(logger.LogLevelInfo, "[系统] 释放内存: %.1f MB (用户=%s)", totalFreedMB, username)
...
func (h *Handler) restartServer(w http.ResponseWriter, r *http.Request) {
	...
	username := h.getSessionUsername(r)
	logger.LogPanelRuntime(logger.LogLevelInfo, "[系统] 服务器重启 (用户=%s)", username)
